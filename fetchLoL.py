...
class RiotClient(object):
	def __init__(self, api_key, realm = 'na', limit=True, max_per_ten_min=500, include_timestamp=True, summonerAPIVersion = "1.4", 
		championAPIVersion = "1.2", gameAPIVersion = "1.3", leagueAPIVersion = "2.5", statsAPIVersion = "1.3", teamAPIVersion = "2.4"):
	...
	def makeapicallonlist(self, url, lst, version):
		...
		try:
		 ...
		 resp = self.makeapicall(url.replace('{}', str(lst)), version)
	...
	def makeapicall(self, remainder, version):
		...
		url = "https://" + urllib2.quote(self.realm) + ".api.pvp.net/api/lol/" + urllib2.quote(self.realm) + "/v" + urllib2.quote(version + remainder) + '?api_key=' + urllib2.quote(self.api_key)
		print "Calling: %s" % url
	...
	def get_mc_league(self, tier):
	 ''' Get all summoners in a league
	 Args:
	  tier (str) : 'master' or 'challenger'
	 Returns:
	  [League]
	 '''
	 resp = self.makeapicall("/league/%s" % tier.lower(), self.leagueAPIVersion)
...
##### (https://github.com/artificium/PyLoL)